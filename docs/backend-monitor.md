...
管理后台的监控人员列表支持「取消监控」和「删除」：取消监控只停用人员，删除会从当前监控配置中移除人员，但不会删除历史任务结果。

已登记的监控会作为长期规则保留。普通触发抢饭时，后端会自动把过期的订餐周推进到当前应订餐的周，并同步到每个用户的规则；用户无需每周重新点击「发起监控」。任务重试不会自动推进订餐周，避免补重试时切到新的周期。
...
抢饭时每个用户内部按所选工作日从周一到周日、同一天内按早餐到晚餐执行；用户之间按监控列表顺序入队，但默认并发执行，不能保证严格按列表从上到下完成。可通过 `ORDER_CONCURRENCY=1` 改为逐个用户执行。
...
- `ORDER_CONCURRENCY`: 并发用户数，默认不限制（启用人员并发执行）
